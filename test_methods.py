...
class TestMethods(unittest.TestCase):
    ...
    def test_deal_cards(self):
        # checks that each player when there are an even number has the right
        # number of cards
        game = Game(['TestOne', 'TestTwo'])
        hands = [len(player.hand) for player in game.players]
        self.assertTrue(hands.count(26) == 2)

        # When there are an odd number of players, checks that hands are the
        # right size and tie_pile has right number of cards
        game = Game(['test1', 'test2', 'test3'])
        hands = [len(player.hand) for player in game.players]
        self.assertTrue(
            hands.count(17) == 3 and len(
                game.card_pile.tie_pile) == 1)
    ...
    def test_deck_building(self):
        ...
        self.assertTrue(
            len([card for card in deck.cards if card.suit.value == 'Spades']) == (52 / 4))
        self.assertTrue(
            len([card for card in deck.cards if card.suit.value == 'Hearts']) == (52 / 4))
        self.assertTrue(
    ...
    def test_remove_players(self):
        ...
        self.assertIsNone(
            next(
                (player for player in game.players if player.name == 'player1'),
                None))
    ...
    def test_card_pile_winner(self):
        ...
        # Testing that the winner has the right amount of cards after
        # collecting winnings
    ...
    def test_player_tie_breaker(self):
        ...
        # Make sure player puts 3 cards in the tie pile if they have more than
        # 3 cards
        player.hand = [
            Card(
                Suits.Clubs, '2'), Card(
